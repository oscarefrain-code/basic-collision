...
computeBBintersections( void )
{
  ...
  int collisionDetec;
  collisionDetec = Bo1_To2.computeRRintersections(); //
  ...
  collisionIndicator = collisionDetec;
  ...
  // //   std::cout << "  (" << Bo1_To2.pointsRR[i].transpose() << ")  ";
  // // std::cout << std::endl;
  ...
  // // for (int i=0; i<pointsBB.size(); i++){ std::cout << "  (" << pointsBB[i].transpose() << ")  "; }
  // // std::cout << std::endl;
...
averageSimilarValues( void )
{
  ...
  for (int i=0; i<pointsBB.size(); i++){
    ...
    if (find(indexToDiscard.begin(), indexToDiscard.end(), i) == indexToDiscard.end())
  	...
  	for (int j=i+1; j<pointsBB.size(); j++){
  	  ...
  	  if ( find(indexToDiscard.begin(), indexToDiscard.end(), j) == indexToDiscard.end())
	      ...
	      std::cout << "distance: " << distance << ", tolerance" << tolerance*tolerance << std::endl;
	...
	Vector3d average;
  	average = 1.0/Nsum*sum;

	//std::cout << "sum: " << sum.transpose() << ", Nsum: " << Nsum << ", Div: " << (1.0/double(Nsum))*sum << std::endl;
...
//   		{
// 		  std::cout << "Dist smaller than tolerance" << std::endl;
//   		  sum(0) += pointsBB[j](0); sum(1) += pointsBB[j](1); sum(2) += pointsBB[j](2);
//   		  Nsum++;
//   		  indexToDiscard.push_back(j);
//   		}
